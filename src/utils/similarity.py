...
def column_name_similarity(a: str, b: str) -> float:
    """
    Computes lexical similarity between normalized column base names.

    Similarity is calculated only on semantic base names,
    ignoring structural administrative tokens.
    """

    a_norm = normalize_column_name(a)
    b_norm = normalize_column_name(b)

    a_base = a_norm["base_name"]
    b_base = b_norm["base_name"]

    if not a_base or not b_base:
        return 0.0

    return SequenceMatcher(None, a_base.lower(), b_base.lower()).ratio()
